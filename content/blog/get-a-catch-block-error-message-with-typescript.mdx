...
The reason for this is because even though in our code it looks like there is no way anything else
could be thrown, JavaScript is a weird language and so its perfectly possible for a third party library
to do something funky like monkey-patching the error constructor to throw something weird:
...
I think the key takeaway here is to remember that while TypeScript has its
